    ...
    public static class SchemaGenerator {
        ...
        public static List<string> Generate(string targetPath, HashSet<Type> allModels, Options options)
        {
            ...
            while (check)
            {
                ...
                var missingFields = new HashSet<Guid>(schema.SelectMany(s => s.Fields).Where(fId => schema.All(l => l.Id != fId)));
                ...
                count++;
                check = schema.SelectMany(s => s.Fields).Any(fId => schema.All(s => s.Id != fId));
            }
